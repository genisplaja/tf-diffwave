...
class MUSDB_3SEC:
    ...
    def load_data(self, data_dir=None):
        ...
        files = tf.data.Dataset.from_tensor_slices(
            [(mix, mix.replace('mixture.', 'vocals.')) for mix in mixture_files])
        # read audio
    ...
    def normalizer(self, frames=16000):
        ...
        Returns:
            Callable, normalizer.
        """
        def normalize(mixture_signal, vocal_signal):
            """Normalize datum.
            Args:
            ...
            return mixture_signal[start:start + frames], vocal_signal[start:start + frames]
    ...
    def mel_val(self, mixture_signal, vocal_signal):
        ...
        logmel = tf.math.log(tf.maximum(mel, self.config.eps))
        # tf.abs(stft) --> magnitude spec of mixture, signal[1] --> vocal signal
        return logmel, mixture_signal, vocal_signal

    def mel_batcher(self, audio_len=66150, input_shape=64):
        """Create LJSpeech normalizer, make fixed size segment in range(-1, 1).
        Args:
            frames: int, segment size, frame unit.
            from_tfds: bool, whether use tfds tfrecord or raw audio.
        Returns:
            Callable, normalizer.
        """
        def mel_batch(logmel, mixture_signal, vocal_signal):
            """Normalize datum.
            Args:
                speech: tf.Tensor, [T], mono audio in range (-1, 1).
            Returns:
            ...
            logmel_shape = math.ceil(audio_len/self.config.hop)
            num_batches = math.floor(logmel_shape/input_shape)
            new_audio_len = num_batches * input_shape * self.config.hop

            feat_list = []
            for i in np.arange(num_batches):
                feat_list.append(logmel[:, i*input_shape:(i+1)*input_shape, :])
