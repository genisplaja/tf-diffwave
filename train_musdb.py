...
class Trainer:
    ...
    def __init__(self, model, lj, config):
        ...
        self.sounds_path = config.train.log.replace('/log', '/sounds')
    ...
    def train(self, step=0, ir_unit=5):
        ...
        for _ in tqdm.trange(step // self.split, self.config.train.epoch):
            ...
            with self.test_log.as_default():
                ...
                filename = os.path.join(self.sounds_path, 'iter_' + str(count) + '.wav')
    ...
    def get_count(self):
        iters = glob.glob(os.path.join(self.sounds_path, 'iter_*'))
        if len(iters) > 2:
