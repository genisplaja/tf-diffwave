...
class DiffWave(tf.keras.Model):
    ...
    def time_diffusion(self, vocals, accomp, alpha):
        """Trans to next state with diffusion process.
        Args:
            signal: tf.Tensor, [B, T], signal.
            alpha_bar: Union[float, tf.Tensor: [B]], cumprod(1 -beta).
        ...
        Return:
            tuple,
                ...
                eps: tf.Tensor, [B, T], noise.
        """
        a = tf.math.scalar_mul(tf.sqrt(alpha), vocals)
        b = tf.math.scalar_mul(tf.sqrt(np.float32(1.0 - alpha)), accomp)
        return tf.add(a, b), accomp
